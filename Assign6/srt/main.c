...
scene_t scene;
int nthreads;
float colorArr[width][height][3];

/* portion of code to be multithreaded */
void* perform_work(void* argument)
{
    ...
    for( int px = tNum; px<width; px += nthreads )
    {
	    ...
	    /* Store values */
	    colorArr[px][py][0] = scaled_color[0];
	    colorArr[px][py][1] = scaled_color[1];
	    colorArr[px][py][2] = scaled_color[2];
	}
    }
    return NULL;
}
...
main( int argc, char **argv )
{
    ...
    for (index = 0; index < nthreads; ++index)
      {
	tid[index] = index;
	result_code = pthread_create(&threads[index], NULL, perform_work, &tid[index]);
	if (result_code)
	  {
	...
	if (result_code)
	  {
	    fprintf(stderr, "In main: thread %d join failure.\n", index);
	    exit(1);
	  }
      }
	...
	for (h = 0; h < height; h++)
	  {
	    printf("%.0f %.0f %.0f\n",
		   colorArr[w][h][0], colorArr[w][h][1], colorArr[w][h][2]);
	  }
	printf("\n");
      }
